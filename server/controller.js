...
const mergeFileChunk = async (filePath, fileHash, size) => {
    ...
    // 根据切片下标进行排序
    console.log('mergeFileChunk ------->', chunkPaths);
    // 否则直接读取目录的获得的顺序可能会错乱
...
module.exports = class {
    ...
    async handleFileChunk(req, res) {
        ...
        multipart.parse(req, async (err, fields, files) => {
            ...
            console.log('handleFileChunk', fields);
            ...
            try {
                await fse.move(chunk.path, path.resolve(chunkDir, `${filename}-${chunkNumber}`));
            } catch (error) {
    ...
    async handleVerifyUpload(req, res) {
        ...
        const ext = extractExt(fileName);
        console.log('handleVerifyUpload', UPLOAD_DIR, `${md5}${ext}`);
        const filePath = path.resolve(UPLOAD_DIR, `${md5}${ext}`);
        const chunkPath = path.resolve(UPLOAD_DIR, `${md5}`);
        if (fse.existsSync(filePath)) {
            return rendAjax(res, {
                code: 200,
                message: '操作成功',
        ...
        } else if (fse.existsSync(chunkPath)) {
            const chunkPaths = await fse.readdir(chunkPath);
            const uploaded = chunkPaths.map(item => Number(item.split('-')[1]));
            return rendAjax(res, {
                code: 200,
                message: '操作成功',
                data: {
